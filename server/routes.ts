...
export async function registerRoutes(
  ...
  // Share/unshare a collection (Venturr)
  app.post("/api/collections/:id/share", isAuthenticated, async (req, res) => {
    try {
      ...
      const updated = await storage.updateCollectionSharing(collectionId, userId, isPublic, shareSlug);
      
      const shareUrl = isPublic && shareSlug 
        ? `${req.protocol}://${req.get('host')}/v/${shareSlug}`
        : null;
  ...
  app.get("/api/v/:slug", async (req, res) => {
    try {
      ...
      const places = await storage.getPlacesForCollection(collection.id);
